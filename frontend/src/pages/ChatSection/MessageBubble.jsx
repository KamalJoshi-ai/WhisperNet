      ...
      >
        <div className="relative group">
              ...
              <div className="flex flex-col">
                ...
                >
                  <img
                    ...
                    alt="Shared image"
                    className="rounded-lg max-w-[200px] mb-2 object-cover cursor-pointer"
                  />
                </a>
          ...
          {showEmojiPicker && !isUserMessage && (
            <div ref={emojiPickerRef} className={`absolute z-50 left-0 top-10`}>
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute top-2 right-1.5 text-gray-500 hover:text-gray-700 transition p-1"
                >
                  <RxCross2 size={18} />
                </button>
              </div>
...
{message.reactions?.length > 0 && (
  <div
    ...
    } flex gap-1 px-1 py-0.5`}
    title="Message reactions"
  >
    {message.reactions.map((reaction, i) => {
     
      const isUserReaction = reaction.user == currentUser._id;
      return (
        <div
        ...
        >
      <span
  className="text-lg cursor-pointer transition hover:scale-210 relative group"
  role="img"
  aria-label={reaction.emoji}
  onClick={() => isUserReaction && setHoveredReaction(prev =>
    prev === reaction.emoji ? null : reaction.emoji
  )}
>
  {reaction.emoji}

  {/* Cross visible on hover (desktop) OR after click (mobile) */}
  {isUserReaction && hoveredReaction === reaction.emoji && (
    <button
      onClick={() => handleRemoveReaction(reaction.emoji)}
      className="absolute -top-3 -right-3 text-red-500 text-xs font-bold p-0.5 "
