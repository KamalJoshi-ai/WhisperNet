...
//     set({socket:socket})
//     //remove existing users to prevent duplicate handlers
...
//       // console.log(reactionUpdated)
//       // let messageId=reactionUpdated.messageId
//       // let reactions = reactionUpdated.reactions
...
//     //handle removed messages from local storage
...
//       set((state) => {
//         const newTypingUsers = new Map(state.typingUsers); // copy old map
...
//         if (isTyping) {
//           typingSet.add(userId); // add user to typing set
//         } else {
//           typingSet.delete(userId); // remove user from typing set
//         }
...
//    socket.on("message-read", (updateMessage) => {
//   set((state) => {
//     const updatedMessages = state.messages.map((msg) => msg._id === updateMessage._id
//         ? { ...msg, messageStatus: updateMessage.messageStatus }
//         : msg
//     );
...
//       set((state) => {
//         const newOnlineUsers = new Map(state.onlineUsers); // copy old map
...
//     });
//     socket.on("Offline",({ userId, isOnline, lastSeen })=>{
//       if(userId == get().Contact){
//     set((state)=>{
//                  const updated = new Map(set.onlineUsers);
...
//     // message belongs to the conversation user is viewing
//     if (message.conversation === currentConversation)
//        {
...
//       // if current user is the receiver, mark as read immediately
//       if ( message.receiver?._id === currentUser?._id &&
//   message.conversation === currentConversation) {
...
//             lastMessage: message,
//             unreadCount:
//               // only increase if user is receiver AND this convo is not opened
//               message?.receiver?._id === currentUser?._id &&
//               message.conversation !== currentConversation
//                 ? (conv.unreadCount || 0) + 1
//                 : conv.unreadCount || 0,
...
//    // get() returns state where all states are in object 
